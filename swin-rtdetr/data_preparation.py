"""
RIVA Part B - Data Preparation Module for Swin-RTDETR
Can be run directly: python data_preparation.py

Usage:
  python data_preparation.py --root_dir riva-partb-dataset --work_dir swin-rtdetr-exp
...
class FoldPreparer:
    ...
    def prepare_fold(
        ...
        """Prepare a single fold with images and YOLO-format labels."""
        ...
        self._process_images(fold_dir, train_images, "train", f"Fold {fold_idx} Train")
        self._process_images(fold_dir, val_images, "val", f"Fold {fold_idx} Val")
    ...
    def _create_yolo_label(
        ...
        """Create YOLO format label file (also used by RT-DETR)."""
    ...
    def _copy_test_images(self, fold_dir: str):
        ...
        if os.path.exists(test_src):
            ...
            print(f"  Warning: Test directory not found: {test_src}")

    def _create_data_yaml(self, fold_dir: str):
        """Create data configuration YAML (same format for YOLO/RT-DETR)."""
...
class DataPreparation:
    ...
    def prepare(self) -> Tuple[List[str], Dict[str, pd.DataFrame]]:
        ...
        print("=" * 80)
        print("RIVA DATA PREPARATION - Swin-RTDETR")
        print("=" * 80)
        ...
        if os.path.exists(self.config.work_dir):
            response = input(f"\n  {self.config.work_dir} exists. Remove it? (y/n): ")
            if response.lower() == 'y':
                shutil.rmtree(self.config.work_dir)
                print(f"  Removed {self.config.work_dir}")
            else:
                print("  Aborted")
                return [], {}
    ...
    def _print_summary(self, fold_dirs: List[str]):
        ...
        print("=" * 80)
        print(f"  Prepared {len(fold_dirs)} folds")
        print(f"\nFold directories:")
...
def verify_dataset_structure(root_dir: str) -> bool:
    ...
    for name, path in required_paths.items():
        ...
        if not exists and name != "test_images":
            all_exist = False
...
def main():
    ...
    parser = argparse.ArgumentParser(description='RIVA Data Preparation - Swin-RTDETR')
