"""
RIVA Part B - Inference Module for Swin-RTDETR
Can be run directly: python inference.py

Usage:
  python inference.py --test_dir path/to/test --models model1.pt model2.pt
  python inference.py --test_dir fold_0/images/test --work_dir swin-rtdetr-exp
"""
...
import swin_backbone  # Required to load Swin-RTDETR checkpoints


class MultiScaleTTAInference:
    """Multi-scale test-time augmentation inference for RT-DETR."""
    ...
    def _predict_at_scale(
        ...
        results = model.predict(
            source=img_path,
            imgsz=scale,
            conf=0.001,       # Low threshold - WBF will filter
    ...
    def _predict_flipped(
        ...
        if results[0].boxes is not None:
            ...
            for box, conf in zip(boxes_xywhn, confs):
                xc, yc, w, h = box
                xc_orig = 1.0 - xc  # Flip x-coordinate back
...
class SubmissionGenerator:
    ...
    def generate_submissions(
        ...
        main_path = os.path.join(output_dir, "swin_rtdetr_submission.csv")
    ...
    def _generate_threshold_versions(
        ...
        for thresh in self.config.final_thresholds:
            ...
            output_path = os.path.join(
                output_dir,
                f"swin_rtdetr_conf_{thresh:.4f}.csv"
            )
            filtered.to_csv(output_path, index=False, float_format='%.6f')

            if len(filtered) > 0:
                avg = len(filtered) / n_images
                print(f"  Threshold {thresh:.4f}: {len(filtered):>6,} predictions ({avg:>6.1f} per image)")

    def _print_submission_stats(self, df: pd.DataFrame):
        """Print submission statistics."""
        print("\n" + "=" * 80)
        print("SWIN-RTDETR SUBMISSION GENERATED")
        print("=" * 80)
        ...
        print("  1. swin_rtdetr_conf_0.0500.csv  (standard threshold)")
        print("  2. swin_rtdetr_conf_0.0100.csv")
        print("  3. swin_rtdetr_conf_0.1000.csv")
        print("  4. swin_rtdetr_conf_0.1500.csv")
        print("=" * 80)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='RIVA Swin-RTDETR Inference')
    parser.add_argument('--test_dir', type=str, required=True,
                       help='Directory containing test images')
    parser.add_argument('--models', type=str, nargs='+', default=None,
                       help='Model paths (space separated)')
    parser.add_argument('--work_dir', type=str, default=None,
                       help='Work directory to auto-find models')
    ...
    parser.add_argument('--wbf_skip', type=float, default=0.001,
    ...
    if args.models:
        ...
        if not model_paths:
            print(f"No models found in {args.work_dir}")
            print(f"Expected: {args.work_dir}/runs/fold_*/weights/best.pt")
            return
    else:
        print("Must specify either --models or --work_dir")
        return
    ...
    print("=" * 80)
    print("RIVA SWIN-RTDETR INFERENCE")
    print("=" * 80)
    print(f"Architecture: Swin Backbone + RT-DETR (end-to-end, no NMS)")
    ...
    print(f"\nSubmissions saved to {args.output_dir}")
